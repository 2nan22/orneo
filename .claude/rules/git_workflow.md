...
`gh pr create`는 반드시 `--base dev`를 명시한다. 누락 시 GitHub 기본 브랜치(main)로 머지되어 히스토리가 오염된다.
...
GitHub 머지 커밋 메시지는 **제목 + 개발 내용 요약 bullet**으로 작성한다.

```
Merge pull request #N from 2nan22/feat/branch-name
...
`gh pr merge` 시 `--subject`와 `--body`로 메시지를 직접 지정한다:
...
## PR 머지 후 클린업 (한 단위로 실행)

```bash
# 1. 머지 + 원격 브랜치 삭제 (위의 gh pr merge 명령으로 한번에)
# 2. dev로 전환 후 동기화
git checkout dev && git pull origin dev
# 3. 로컬 feature 브랜치 삭제
...
세 단계를 반드시 한 세트로 실행한다. 머지 직후 dev가 최신 상태인지 확인하는 것이 목적이다.
...
gh pr create --base dev ...        ← 반드시 --base dev 명시
  ↓
gh pr merge <N> --merge --delete-branch --subject ... --body ...
git checkout dev && git pull origin dev
git branch -d feat/english-branch-name
  ↓
...
[기능 완성 마일스톤 도달 시]
  ↓
dev 통합 테스트 통과
gh pr create --base main ...       ← dev → main PR
